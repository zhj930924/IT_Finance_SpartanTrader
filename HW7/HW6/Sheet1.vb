...
Public Class Sheet1
    ...
    Private Sub ComputeCagrBtn_Click(sender As Object, e As EventArgs) Handles ComputeCagrBtn.Click
        Dim LastRow As Integer = Cells(Rows.Count, "G").End(Excel.XlDirection.xlUp).Row
        Dim CAGR As Double = 0
        Dim FinalPrice As Double = 0
        Dim InitPrice As Double = 0
        Dim Days As Double = 0
        Dim MyTS As TimeSpan
        Dim AvgCAGR As Double = 0

        For i As Integer = 1 To LastRow
            FinalPrice = Cells(i, "J").Value
            InitPrice = Cells(i, "H").Value
            MyTS = Cells(i, "I").Value - Cells(i, "G").Value
            Days = MyTS.Days
            CAGR = CalcCAGR(FinalPrice, InitPrice, Days)
            Cells(i, "K").Value = CAGR.ToString("P3")
        Next

        AvgCAGR = Application.WorksheetFunction.Average(Range("K1:K" & LastRow.ToString()))
        Cells(LastRow + 2, "K").Value = AvgCAGR.ToString("P3")
        Cells(LastRow + 2, "L").Value = "Average CAGR"
    End Sub

    Private Function CalcCAGR(FP As Double, IP As Double, days As Double) As Double
        Return ((FP / IP) ^ (1 / days)) - 1
    End Function
End Class
